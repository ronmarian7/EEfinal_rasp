...
def config_feeder(dtpin1=19, sckpin1=13, dtpin2=23, sckpin2=24, sleeptime=2, config_range=4, referenceUnit=1,portions=10,feederpin=26):
    try:
        ...
        for i in range(config_range):
            ...
            print(f"Config food's weight1 is: {food_weight1} gr.")
            food_weight2 = max(0, int(hx2.get_weight()))
            print(f"Config food's weight2 is: {food_weight2} gr.")
        ...
        print("filling bowl for the first time...")
        seconds = portions * 4
        ...
        food_weight2 = max(0, int(hx2.get_weight()))
        total_food_weight = food_weight1 + food_weight2
        print(f"Config food's weight after feeding is: {total_food_weight} gr.")
...
def handle_feeder(hx1, hx2, sleeptime=4, feedingthreshold=100, cur_ref_weight=500, portions=4,
    ...
    while total_food_weight is None:
        try:
            ...
            print(f"Food's reference weight is: {cur_ref_weight} gr.")
...
if __name__ == "__main__":
    ...
    hx1, hx2, config_weight = config_feeder(dtpin1=FOOD_WEIGHT_DTPIN1, sckpin1=FOOD_WEIGHT_SCKPIN1,
                                            ...
                                            referenceUnit=FOOD_WEIGHT_REFERENCE_UNIT,portions=FEEDER_PORTIONS,feederpin=FEEDER_PIN)
    while True:
